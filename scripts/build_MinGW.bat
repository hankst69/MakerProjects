...
if "%_MGW_BUILD_TYPE%"    equ "" set _MGW_BUILD_TYPE=Release
if "%_MGW_TGT_ARCH%"      equ "" set _MGW_TGT_ARCH=x64
...
rem set "_MGW_DIR=%MAKER_BIN%\mingw"
...
if not exist "%_MGW_BIN_DIR%\gcc.exe" (
...
:test_MGW_succes
call "%MAKER_BUILD%\validate_mingw.bat" %_MGW_VERSION% %_MGW_BUILD_TYPE% %_MGW_TGT_ARCH% --no_warnings --no_errors --no_info
if %ERRORLEVEL% EQU 0 goto :exit_script
if "%MAKER_ENV_VERBOSE%" neq "" echo on
...
set _BMGW_START_DIR=
call "%MAKER_BUILD%\validate_mingw.bat" %_MGW_VERSION% %_MGW_BUILD_TYPE% %_MGW_TGT_ARCH% %MAKER_ENV_VERBOSE% --no_warnings