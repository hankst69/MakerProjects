...
if exist "%MAKER_ENV_BIN%\make.bat" (
  ...
  set _VERSION_NR=
  for /f "tokens=1,2,* delims= " %%i in ('call "%%MAKER_ENV_BIN%%\make.bat" --version') do if /I "%%i" equ "gnu" if /I "%%j" equ "make" set "_VERSION_NR=%%k"
  rem if "%_VERSION_NR%" neq "" echo MAKE already available
  if "%_VERSION_NR%" neq "" set "Path=%MAKER_ENV_BIN%;%Path%"
