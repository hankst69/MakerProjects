...
set ERRORLEVEL=
call "%MAKER_DIR_SCRIPTS%\validate_gperftools.bat" %_BISON_VERSION% 1>nul 2>nul
...
call "%MAKER_DIR_SCRIPTS%\clone_gperftools.bat" %_GPT_VERSION%
...
call "%MAKER_DIR_SCRIPTS%\ensure_msvs.bat" GEQ2019 amd64
if %ERRORLEVEL% NEQ 0 (
  goto :Exit
)
rem validate cmake
...
cd "%_GPT_BUILD_DIR%"
call cmake --build . --config %_GPT_BUILD_TYPE% --parallel %MAKER_NUM_PARALLEL%
...
call "%MAKER_DIR_SCRIPTS%\validate_gperftools.bat" %_GPT_VERSION% 1>nul 2>nul
if %ERRORLEVEL% NEQ 0 set "PATH=%PATH%;%_GPT_BIN_DIR%\bin"

:Exit
cd "%_GPT_DIR%"
"%MAKER_DIR_SCRIPTS%\validate_gperftools.bat" %_GPT_VERSION%
