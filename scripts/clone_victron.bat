...
set "VICTRON_SILENT_CLONE_MODE=%MAKER_MSG_SILENT%"
set "VICTRON_CHECKOUT_TAG=--checkoutTag %VICTRON_GUIV2_VERSION%"
if "%VICTRON_GUIV2_VERSION%" equ "" set VICTRON_CHECKOUT_TAG=

if "%MAKER_MSG_VERBOSE%" neq "" set VICTRON_

call "%MAKER_ENV_CORE%\clone_in_folder.bat" "%VICTRON_GUIV2_SRC_DIR%" "https://github.com/victronenergy/gui-v2.git" %VICTRON_CHECKOUT_TAG% %VICTRON_SILENT_CLONE_MODE%
rem we also have to clone the needed submodules (veutil and qzxing ..)
pushd "%VICTRON_GUIV2_SRC_DIR%"
call git submodule update --init
popd

call "%MAKER_ENV_CORE%\clone_in_folder.bat" "%VICTRON_HTMLAPP_DIR%" "https://github.com/victronenergy/venus-html5-app.git" %VICTRON_SILENT_CLONE_MODE%

rem set "VICTRON_VENUSOS_SRC_DIR=%_VICTRON_DIR%\venus-os_source"
rem call "%MAKER_ENV_CORE%\clone_in_folder.bat" "%VICTRON_VENUSOS_DIR%" "https://github.com/victronenergy/venus.git" %VICTRON_SILENT_CLONE_MODE%
