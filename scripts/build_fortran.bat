...
set "_FRT_ENV_DIR=%MAKER_BIN%\.ifort_env"

if "%_REBUILD%" neq "" (
  del /F /Q "%MAKER_BIN%\gfortran.bat" 2>NUL
  del /F /Q "%MAKER_BIN%\ifort.bat" 2>NUL
  del /F /Q "%MAKER_BIN%\ifx.bat" 2>NUL
...
rem if exist "%MAKER_BIN%\gfortran.bat" 
call "%MAKER_BIN%\gfortran.bat" --validate 1>nul 2>nul
if %ERRORLEVEL% EQU 0 goto :test_fortran
set _FRT_TOOL=ifort.bat
call "%MAKER_BIN%\ifort.bat" --validate 1>nul 2>nul
if %ERRORLEVEL% EQU 0 goto :test_fortran
set _FRT_TOOL=ifx.bat
call "%MAKER_BIN%\ifx.bat" --validate 1>nul 2>nul
if %ERRORLEVEL% EQU 0 goto :test_fortran
...
echo @if /I "%%~1" equ "--validate" ^(exit /b 0^)>"%MAKER_BIN%\gfortran.bat"
echo @call "%_MGW_BIN_DIR%\gfortran.exe" %%*>>"%MAKER_BIN%\gfortran.bat"
goto :test_fortran
...
echo @call "%_FRT_ENV_DIR%\Scripts\activate" >>"%MAKER_BIN%\ifort.bat"
...
rem
echo @if /I "%%~1" equ "--validate" ^(exit /b 0^)>"%MAKER_BIN%\ifort.bat"
echo @call ifort %%*>>"%MAKER_BIN%\ifort.bat"
rem type "%MAKER_BIN%\ifort.bat"
goto :test_fortran
