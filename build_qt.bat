...
call "%~dp0.\clone_qt.bat" %_QT_VERSION%
...
if not exist "%_QT_BIN_DIR%\bin\Qt6WebSockets.dll" (
  ...
  call cmake --install .
  popd
  if not exist "%_QT_BIN_DIR%\bin\Qt6WebSockets.dll" echo error: QT-INSTALL FAILED&goto :EOF
)
call which Qt6WebSockets.dll 1>nul 2>nul
if %ERRORLEVEL% EQU 0 echo QT-INSTALL done&goto :qt_install_done
